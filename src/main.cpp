...
#if defined(NDEBUG)
# error "Acoin7 cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x93e795b36e5ae0fc793cde771f80fa84b5301cf9167a9d19c6ddaf577e777ed8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Acoin7: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Acoin7 Signed Message:\n";
...
{
    // Acoin7: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Acoin7: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Acoin7: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Acoin7: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Acoin7: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Acoin7: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbf8c3bda32c450e3bfb4669679c79681814a8b2c14fd0694fc40525ccfb8464f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Tashkent 2022/04/16 yaritilgan Asia Blockchain System";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040cd0c04cfc8365d62258723e51d462e0825b5ca49e6b9f67b75380e3d56b1f7a3cde728d5fc191d43d2d822c4243610e0138662485b053762c988731330a8edf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1650119516;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085705707;

        if (fTestNet)
        {
            block.nTime    = 1650119499;
            block.nNonce   = 385581764;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x559922ee1001e47bc2bd6dc83d0742d6574d2b41952a96bf427b878729a07c1d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb7, 0xdc }; // Acoin7: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Acoin7Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Acoin7Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Acoin7Miner(CWallet *pwallet)
{
    printf("Acoin7Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("acoin7-miner");
    ...
    try { loop {
        ...
        printf("Running Acoin7Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Acoin7Miner, pwallet));
}
